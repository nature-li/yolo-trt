...
std::vector<Detection> Detector::detect(const cv::Mat& img, float conf_thresh) {
  ...
  // 2. H2D
  cudaMemcpy(d_input_, chw.data, 1 * 3 * INPUT_H * INPUT_W * sizeof(float),
  ...
  // 4. D2H
  int out_size = 1 * (NUM_CLS + 4) * NUM_DET;
  std::vector<float> h_output(out_size);
  cudaMemcpy(h_output.data(), d_output_, out_size * sizeof(float),
             cudaMemcpyDeviceToHost);

  // 5. 后处理
  return postprocess(h_output.data(), img.cols, img.rows, conf_thresh);
}